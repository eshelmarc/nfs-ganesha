...
static bool reset_nfsv_export_total_ops(DBusMessageIter *args,
			 		DBusMessage *reply)
{
	struct gsh_export *export = NULL;
	struct export_stats *export_st = NULL;
	...
	dbus_status_reply(&iter, success, errormsg);
	if (success)
		server_dbus_reset_total_ops(export_st);
...
static bool reset_nfsv_global_reset_ops(DBusMessageIter *args,
			 		DBusMessage *reply)
{
	bool success = true;
	char *errormsg = "OK";
	...
	server_dbus_reset_global_ops(&iter);
...
static struct gsh_dbus_method export_reset_total_ops = {
	.name = "ResetTotalOPS",
	.method = reset_nfsv_export_total_ops,
	.args = {EXPORT_ID_ARG,
		 STATUS_REPLY,
		 END_ARG_LIST}
};

static struct gsh_dbus_method global_reset_total_ops = {
	.name = "ResetGlobalOPS",
	.method = reset_nfsv_global_reset_ops,
	.args = {EXPORT_ID_ARG,
		 STATUS_REPLY,
		 END_ARG_LIST}
};
...
static struct gsh_dbus_method *export_stats_methods[] = {
	...
	&export_reset_total_ops,
	&export_show_9p_io,
	&global_show_total_ops,
	&global_reset_total_ops,
	NULL
};
